defmodule ExmeraldaWeb.CoreComponents do
  @moduledoc """
  ...
  def button(assigns) do
    ~H"""
    <button type={@type} class={["phx-submit-loading:opacity-75 btn", @class]} {@rest}>
      {render_slot(@inner_block)}
    </button>
    """
  end
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div>
      ...
      <select
        id={@id}
        name={@name}
        class="mt-2 block w-full rounded-md border border-gray-300 bg-white shadow-sm focus:border-zinc-400 focus:ring-0 sm:text-sm text-zinc-900"
        multiple={@multiple}
        {@rest}
      >
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(ExmeraldaWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(ExmeraldaWeb.Gettext, "errors", msg, opts)
    end
  end
